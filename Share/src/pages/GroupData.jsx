...
export default function GroupData() {
  ...
  const uploadSignature = async (file) => {
    ...
    try {
      ...
      if (newSignature) {
        const signatureUrl = newSignature.startsWith('http') ? newSignature : `${API_BASE_URL}/${newSignature}`;
        ...
        try {
          ...
          if (backendData.signature) {
            const signatureUrl = backendData.signature.startsWith('http') ? backendData.signature : `${API_BASE_URL}/${backendData.signature}`;
            setSignaturePreview(signatureUrl);
            setForm((prev) => ({ ...prev, signature: signatureUrl }));
            setData((prev) => ({ ...prev, signature: signatureUrl }));
