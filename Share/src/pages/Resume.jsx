...
export default function Resume() {
  ...
  // Test API endpoint to see what fields are expected
  const testResumeAPI = async () => {
    try {
      const token = localStorage.getItem("token");
      const uid = localStorage.getItem("uid");
      
      console.log('Testing resume API endpoint...');
      
      // Create a minimal test FormData
      const testFormData = new FormData();
      testFormData.append('name', 'Test User');
      testFormData.append('contact', '1234567890');
      testFormData.append('email_id', 'test@example.com');
      testFormData.append('qualification', 'Test Qualification');
      testFormData.append('skill', 'Test Skill');
      testFormData.append('skills', 'Test Skills');
      testFormData.append('experience', '1 year');
      
      // Create a dummy file
      const dummyFile = new File(['test content'], 'test.txt', { type: 'text/plain' });
      testFormData.append('resume_file', dummyFile);
      ...
      const response = await api.post("/resume/add", testFormData, {
        headers: {
          "Client-Service": "COHAPPRT",
          "Auth-Key": "4F21zrjoAASqz25690Zpqf67UyY",
          uid: uid,
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // Test: Verify skills field specifically
      console.log('Skills field verification:');
      console.log('- uploadFormData.skills:', uploadFormData.skills);
      console.log('- Type of skills:', typeof uploadFormData.skills);
      console.log('- Skills length:', uploadFormData.skills?.length);
      console.log('- Skills trimmed:', uploadFormData.skills?.trim());
      console.log('- Skills in FormData:', formData.get('skill'));

      console.log('Uploading resume with data:', {
        name: uploadFormData.fullName,
        email_id: uploadFormData.emailAddress,
        contact: uploadFormData.contactNumber,
        qualification: uploadFormData.qualification,
        skill: uploadFormData.skills,  // This should not be empty
        experience: uploadFormData.experience,
        fileName: uploadFormData.resumeFile.name,
        fileSize: uploadFormData.resumeFile.size
      });
      ...
      toast.success("Resume uploaded successfully!");
        ...
        console.log('Upload response format unexpected:', response.data);
  ...
  const handleViewResume = (resume) => {
    ...
    try {
      ...
      console.log("Opening resume file:", fileUrl);
  ...
  const handleViewResumeWithOptions = (resume, event) => {
    ...
    if (choice) {
      // First option (View in Browser for PDF, Download for others)
      if (fileExtension === 'pdf') {
        window.open(fileUrl, '_blank');
        toast.success("Opening PDF resume...");
      } else {
        ...
        <div className="rounded-2xl shadow-lg bg-white dark:bg-gray-800 max-w-7xl w-full mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 px-6 py-4 border-b border-gray-100 dark:border-gray-700">
            ...
            <div className="flex flex-wrap gap-2 items-center justify-between xl:justify-start">
              ...
              <button 
                className="flex items-center gap-1 bg-yellow-500 text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-yellow-600 transition"
